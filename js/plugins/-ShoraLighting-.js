/*:
 * @plugindesc 
 * [v1.9.7] Provide dynamic lighting to RPG Maker MV/MZ engine, intended to be easiest to start and most flexible when advanced! 
 * @author Shora
 ...
 * @text [Lighting: Default Status]
 * @desc Default status of the lighting
 * @type boolean
 * @default true
 * 
 * @param battleLightingStatus
 * @text [Battle Lighting: Default Status]
 * @desc Default status of the battle lighting
 * @type boolean
 * @default true
 * 
 * @param sep
 ...
 * @default {"status":"false","sep0":"","brightness":"1.25"}
 */
...
((_) => {
    ...
    const createUpperLayer = _.createUpperLayer;
    _.createUpperLayer = function() {
        if (!$gameLighting._disabled && $gameLighting._battleLightingStatus)
            $shoraLayer.loadScene(this);
        createUpperLayer.call(this);
    }
...
class LightingLayer {
    ...
    initialize(spritesetType) {
        ...
        this._spritesetType = spritesetType;
    ...
    addBattleLightingSprite() {
        ...
        for (let i = 0; i < members.length; ++i) {
            let actor = $shoraLayer._spriteset._actorSprites[i];

            const param = $shoraLayer.LIGHTING['default']; param.shadow = false;
            const light = new BattleLightingSprite(param, actor);
            this.layer.addChild(light);

        }
        for (let i = 0; i < $shoraLayer._spriteset._enemySprites.length; ++i) {
            let actor = $shoraLayer._spriteset._enemySprites[i];
...
class BattleLightingSprite extends LightingSprite {
...
class Shadow {
    constructor(lighting, shadowAmbient) {
        ...
        this.texture = PIXI.RenderTexture.create(lighting.width, lighting.height);
        this.sprite = new PIXI.Sprite(this.texture);
        this.sprite.blendMode = PIXI.BLEND_MODES.MULTIPLY;
...
GameLighting.prototype.loadParameters = function() {
    this._disabled = Shora.Lighting.PARAMETERS.lightingStatus == 'false';
    ...
    this._battleLightingStatus = Shora.Lighting.PARAMETERS.battleLightingStatus == 'true';
}
...
class GameShadow {
    ...
    scanMapCaster() {
        const width = $gameMap.width(),
              ...
              wallID = $shoraLayer._wallID,
              topID = $shoraLayer._topID;
