...
(function (DotMoveSystem) {
    ...
    class CharacterCollisionChecker {
        ...
        enteringMassesCharacters(x, y) {
            ...
            // Optimization for maps with many events
            if ($gameMap.events().length > 50) {
                // Only check masses near the character's current position
                const centerX = Math.floor(this._character._realX);
                const centerY = Math.floor(this._character._realY);
                const checkRange = 1; // Adjust based on performance needs
                
                // Create a smaller range to check
                for (let ix = centerX - checkRange; ix <= centerX + checkRange; ix++) {
                    for (let iy = centerY - checkRange; iy <= centerY + checkRange; iy++) {
                        const ix2 = $gameMap.roundX(ix);
                        const iy2 = $gameMap.roundY(iy);
                        if ($gameMap.isValid(ix2, iy2)) {
                            const massIdx = iy2 * $gameMap.width() + ix2;
                ...
                // Original implementation for fewer events
    ...
    Game_Map.prototype.update = function (sceneActive) {
        ...
        $gamePlayer.prepareUpdate();
        for (const follower of $gamePlayer.followers().data()) {
            follower.prepareUpdate();
        }

        for (const event of this.events()) {
            const distance = $gameMap.distance(event.x, event.y, $gamePlayer.x, $gamePlayer.y);

            // Update strategy based on distance:
            // - Close events (< 5 tiles): update every frame
            // - Medium distance (5-10 tiles): update every 2 frames
            // - Far events (10-15 tiles): update every 3 frames
            // - Very far events (> 15 tiles): update every 5 frames
            if (distance < 15 ||
                (distance < 25 && this._updateFrameCount % 2 === 0) ||
                (distance < 35 && this._updateFrameCount % 3 === 0) ||
                (this._updateFrameCount % 5 === 0)) {
                event.prepareUpdate();
            }
        }

        //$gameMap.boat().prepareUpdate();
        //$gameMap.ship().prepareUpdate();
        //$gameMap.airship().prepareUpdate();
