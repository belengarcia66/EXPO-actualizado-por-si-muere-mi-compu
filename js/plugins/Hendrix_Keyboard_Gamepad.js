 ...
 * Ok, Cancel, PageDown, PageUp, Control, Shift, Tab, Escape,
 * Up, Left, Right, Down
 * 
 ...
 * @default ["{\"DisplayName\":\"Move Up\",\"From Key\":\"Up\",\"To Key\":\"W\",\"Gamepad Key\":\"Up\",\"Visibility\":\"Hide in Button Config\",\"a\":\"\",\"aa\":\"\",\"b\":\"\",\"Icon\":\"0\",\"Common Event ID\":\"0\",\"Options\":\"triggered\",\"AllowParallel\":\"false\"}","{\"DisplayName\":\"Move Left\",\"From Key\":\"Left\",\"To Key\":\"A\",\"Gamepad Key\":\"Left\",\"Visibility\":\"Hide in Button Config\",\"a\":\"\",\"aa\":\"\",\"b\":\"\",\"Icon\":\"0\",\"Common Event ID\":\"\",\"Options\":\"triggered\",\"AllowParallel\":\"false\"}","{\"DisplayName\":\"Move Right\",\"From Key\":\"Right\",\"To Key\":\"D\",\"Gamepad Key\":\"Right\",\"Visibility\":\"Hide in Button Config\",\"a\":\"\",\"aa\":\"\",\"b\":\"\",\"Icon\":\"0\",\"Common Event ID\":\"\",\"Options\":\"triggered\",\"AllowParallel\":\"false\"}","{\"DisplayName\":\"Move Down\",\"From Key\":\"Down\",\"To Key\":\"S\",\"Gamepad Key\":\"Down\",\"Visibility\":\"Hide in Button Config\",\"a\":\"\",\"aa\":\"\",\"b\":\"\",\"Icon\":\"0\",\"Common Event ID\":\"\",\"Options\":\"triggered\",\"AllowParallel\":\"false\"}","{\"DisplayName\":\"Interact\",\"From Key\":\"Ok\",\"To Key\":\"E, Enter, Z\",\"Gamepad Key\":\"A\",\"Visibility\":\"Show in Button Config\",\"a\":\"\",\"aa\":\"\",\"b\":\"\",\"Icon\":\"0\",\"Common Event ID\":\"\",\"Options\":\"triggered\",\"AllowParallel\":\"false\"}","{\"DisplayName\":\"Cancel\",\"From Key\":\"Cancel\",\"To Key\":\"X, Esc\",\"Gamepad Key\":\"B\",\"Visibility\":\"Show in Button Config\",\"a\":\"\",\"aa\":\"\",\"b\":\"\",\"Icon\":\"0\",\"Common Event ID\":\"\",\"Options\":\"triggered\",\"AllowParallel\":\"false\"}"]
 * 
...
(function () {
    ...
    const originalButtonMappings = customButtons.map(button => ({
        fromKey: button['From Key'],
        keyboardKey: button['To Key'] || button['From Key'],
        gamepadKey: button['Gamepad Key'] || ''
    }));
    ...
    function remapKeys() {
        ...
        // Start with default mapping then change stuff later
    ...
    function triggerCommonEvent(option, key, mouseButton, gamepadButton, commonEventId, allowParallel, sceneMap) {
        const isActive = checkInputActive(option, key, mouseButton, gamepadButton);
        if (isActive) {
            if (allowParallel) {
    ...
    function checkInputActive(option, key, mouseButton, gamepadButton) {
        if (option === 'triggered') {
            if (key && Input.isTriggered(key)) return true;
            if (mouseButton && getMouseButtonState(mouseButton, 'triggered')) return true;
            if (gamepadButton && gamepadButton !== 'none' && GamepadButtons[gamepadButton] !== undefined) {
                const gpButtonName = Input.gamepadMapper[GamepadButtons[gamepadButton]];
                if (gpButtonName && Input.isTriggered(gpButtonName)) return true;
            }
        }
        else if (option === 'pressed') {
            if (key && Input.isPressed(key)) return true;
            if (mouseButton && getMouseButtonState(mouseButton, 'pressed')) return true;
            if (gamepadButton && gamepadButton !== 'none' && GamepadButtons[gamepadButton] !== undefined) {
                const gpButtonName = Input.gamepadMapper[GamepadButtons[gamepadButton]];
                if (gpButtonName && Input.isPressed(gpButtonName)) return true;
            }
        }
        return false;
    }
        ...
        customButtons.forEach(button => {
            ...
            for (const keyboardKey of keyboardKeys) {
                if (keyboardKey) {
                    ...
                    if (keyActive) {
                        triggerCommonEvent(option, normalizedKey, null, null, commonEventId, allowParallel, sceneMap);
                        return;
                    }
                }
            }
            ...
            for (const gamepadKey of gamepadKeys) {
                if (gamepadKey) {
                    ...
                    if (gpButtonIndex !== undefined) {
                        ...
                        if (buttonActive) {
                            triggerCommonEvent(option, gpButtonName, null, null, commonEventId, allowParallel, sceneMap);
                            return;
                        }
                    }
                }
    ...
    Scene_ButtonConfig.prototype.resetToDefaultConfig = function () {
        customButtons.forEach((button, index) => {
            const originalMapping = originalButtonMappings.find(
                mapping => mapping.fromKey === button['From Key']
            );

            if (originalMapping) {
                button['To Key'] = originalMapping.keyboardKey;
                button['Gamepad Key'] = originalMapping.gamepadKey;
            } else {
                button['To Key'] = button['From Key'];
                button['Gamepad Key'] = '';
            }
        });
    ...
    Scene_ButtonConfig.prototype.processNewBinding = function (keyName) {
        ...
        if (customIndex !== undefined && customIndex !== null) {
            ...
            if (customButton) {
                ...
                if (this._captureCategory === "keyboard") {
                    // Find if this key is already used by another button
    ...
    ConfigManager.makeData = function () {
        ...
        config.buttonConfig = JSON.stringify(customButtons);
        return config;
    };
    ...
    ConfigManager.applyData = function (config) {
        ...
        if (config.buttonConfig) {
            const savedButtons = JSON.parse(config.buttonConfig);
            customButtons.forEach((button, index) => {
                const savedButton = savedButtons.find(b => b['From Key'] === button['From Key']);
                if (savedButton) {
                    button['To Key'] = savedButton['To Key'];
                    button['Gamepad Key'] = savedButton['Gamepad Key'];
                }
            });
            parameters['Button Config'] = JSON.stringify(customButtons);
            remapKeys();
